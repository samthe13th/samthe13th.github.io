...
var balltrans, coordstrans, currentSpeed, newpos, newdx, newdy, cVy;
var drag = function () {
    ...
    if (slider.sliderPoint === maxTime) {
        ...
        cVy = Math.round(100 * (-1 * getVy(vy, 9.81, (slider.sliderPoint / 10)))) / 100;
        currentSpeed = Math.round(10 * Math.sqrt(Math.pow(cVy, 2) + Math.pow(vx, 2))) / 10;
    }
    //     //var newdx = 1;
    //     newdx = newpos.x - ball.x;
    //     newdy = newpos.y - ball.y;
    balltrans = "T" + newpos.x + "," + newpos.y;
    // coordstrans = "T" + (newpos.x + 40) + "," + (newpos.y - 20);
    //   //  console.log("BALL.X = " + ball.x + " NEWPOS.X = " + newpos.x + " BALL.Y = " + ball.y);
    //     // ball.translate(newdx, newdy);
    //   //  console.log("send ball to: " + balltrans);
    ball.transform(balltrans);
    //coords.transform(coordstrans);
...
var ball;
//console.log("make ball at " + startx + starty);
ball = sandbox.circle(0, 0, 10).attr({ stroke: "#ffffff", "stroke-width": 2 });
ball.transform("T" + startx + "," + starty);
...
var coords = sandbox.text(0, 0, "(0,100)").attr({ "fill": "white", "font-size": 18 });
coords.transform("T" + (startx + 40) + "," + (starty - 20));
coords.x = startx + 40;
coords.y = starty - 20;
yline.y = starty;
